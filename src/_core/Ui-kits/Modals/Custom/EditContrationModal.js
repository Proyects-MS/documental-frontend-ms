...
const EditContrationModal = ({ modal, toggle, changing, item }) => {
  ...
  const getData = async () =>{
    ...
    reset({
      name: item.name,
      processtype: item.id_processtype ? item.id_processtype.id : '' 
     })
  ...
  const showAlert = (data) =>{
    SweetAlert.fire({
        title: 'Seguro?',
        text: 'Desea modificar la Contratación!',
        icon: 'warning',
  ...
  const onSubmit = async (data) => {
    ...
    if (data !== '') {
      await axios({
        ...
        changing();
        toast.success(`Contratacion modificado correctamente`);
        reset();
        toggle();
        setIsSaving(false);
      }).catch((error) =>{
  ...
  <option value=''>Seleccione</option>
  {
    TypeContration.map((item) => (
      <option key={item.id} value={item.id}>{item.name}</option>
                  ...
                  <span>{errors.id_processtype && '*Requerido'}</span>
                  <div className="valid-feedback">{'Correcto'}</div>
                </FormGroup>
...
export default EditContrationModal;