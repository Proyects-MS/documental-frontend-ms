...
const EditTaskModal = ({ modal, toggle, changing, id }) => {
    ...
    <Modal isOpen={modal} toggle={toggle} centered size='lg'>
      ...
      <Form className="theme-form mega-form needs-validation" noValidate="" onSubmit={handleSubmit(showAlert)}>
            ...
            <div className='row'>
              ...
              <div className='col-md-6'>
                <FormGroup>
                  ...
                  <select className="form-control" name="file" {...register('file', { required: false })} onChange={(e) => handleSelectFile(e.target.value)}>
                    <option value={''}>Seleccione...</option>
                    {
                      files.map((item) =>{
                        return(
        ...
        <ModalFooter>
          ...
          {
            !saving ? 
            <Button color='primary' disabled={data.status === 'C' ? true : false}><i className="icofont icofont-diskette"></i> Guardar</Button> : 
