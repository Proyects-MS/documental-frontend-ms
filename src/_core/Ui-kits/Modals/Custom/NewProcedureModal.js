...
const NewProcedureModal = ({ modal, toggle, changing }) => {
    ...
    const showAlert = (data) => {
        SweetAlert.fire({
            title: 'Seguro?',
            text: 'Desea agregar Procedimiento!',
            icon: 'warning',
    ...
    const onSubmit = async (data) => {
        ...
        if (data !== '') {
            ...
            bodyFormDataProfile.append('id_hiring', data.id_hiring);
        } else {
            setSaving(false);
            errors.showMessages();
            return
        ...
        await axios({
            ...
            changing();
            toast.success('Procedimiento agregada con éxito');
            reset();
            toggle();
            setSaving(false);
        }).catch((error) => {
    ...
    return (
        <Modal isOpen={modal} toggle={toggle} centered size='lg'>
            <ModalHeader toggle={toggle}>{'Agregar Procedimiento'}</ModalHeader>
            <Form onSubmit={handleSubmit(showAlert)}>
                <ModalBody>
                    ...
                    <FormGroup>
                        <Label for="id_hiring">Contratación</Label>
                        <InputGroup>
                            <select
                                className="form-control"
                                type="select"
                        ...
                        <span className="text-danger text-small d-block mb-2">
                            {errors.id_hiring && 'Contratación es requerido'}
                        </span>
                    </FormGroup>
                </ModalBody>
                <ModalFooter>
